...
from dalle.dalle2_pytorch import Decoder, DiffusionPrior, DiffusionPriorNetwork, CLIP
...
def split_args_and_kwargs(x, *args, split_size=None, **kwargs):
    ...
    split_all_args = [
        split(arg, split_size=split_size) if exists(arg) and isinstance(arg, (torch.Tensor, Iterable)) else (
                (arg,) * chunk_size) for arg in all_args]
...
def load_clip_model(dprior_path, device):
    ...
    clip_config = loaded_obj['hparams']['clip_network']
...
class DiffusionPriorTrainer(nn.Module):
    ...
    def forward(
        ...
        for chunk_size_frac, (chunked_args, chunked_kwargs) in split_args_and_kwargs(x, *args,
...
class DecoderTrainer(nn.Module):
    ...
    def forward(
        ...
        for chunk_size_frac, (chunked_args, chunked_kwargs) in split_args_and_kwargs(x, split_size=max_batch_size,
                                                                                     **kwargs):
            with autocast(enabled=self.amp):
