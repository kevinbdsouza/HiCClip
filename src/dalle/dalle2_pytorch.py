...
from dalle.tokenizer import tokenizer
from dalle.vqgan_vae import NullVQGanVAE, VQGanVAE
from dalle.resize_right import resize
...
class Unet(nn.Module):
    def __init__(
        ...
        if cond_on_text_encodings:
            self.text_to_cond = nn.LazyLinear(cond_dim) if not exists(text_embed_dim) else nn.Linear(text_embed_dim,
                                                                                                     cond_dim)
        ...
        self.mid_attn = EinopsToAndFrom('b c h w', 'b (h w) c',
                                        Residual(Attention(mid_dim, **attn_kwargs))) if attend_at_middle else None
...
class Decoder(BaseGaussianDiffusion):
    def __init__(
            ...
            *,
            ...
            # the paper suggested that this didn't do much in the decoder, but i'm allowing the option for experimentation
            clip_denoised=True,
            clip_x_start=True,
            clip_adapter_overrides=dict(),
        ...
        assert not (
                    condition_on_text_encodings and unconditional), 'unconditional decoder image generation cannot be set to True if conditioning on text is present'

        assert exists(clip) ^ exists(
            image_size), 'either CLIP is supplied, or you must give the image_size and channels (usually 3 for RGB)'
        ...
        assert lowres_conditions == (False, *((True,) * (len(
            self.unets) - 1))), 'the first unet must be unconditioned (by low resolution image), and the rest of the unets must have `lowres_cond` set to True'
    ...
    def p_mean_variance(self, unet, x, t, image_embed, text_encodings=None, text_mask=None, lowres_cond_img=None,
                        clip_denoised=True, predict_x_start=False, learned_variance=False, cond_scale=1.,
                        model_output=None):
    ...
    @torch.inference_mode()
    def p_sample(self, unet, x, t, image_embed, text_encodings=None, text_mask=None, cond_scale=1.,
    ...
    @torch.inference_mode()
    def p_sample_loop(self, unet, shape, image_embed, predict_x_start=False, learned_variance=False, clip_denoised=True,
                      lowres_cond_img=None, text_encodings=None, text_mask=None, cond_scale=1):
        device = self.betas.device
    ...
    def p_losses(self, unet, x_start, times, *, image_embed, lowres_cond_img=None, text_encodings=None, text_mask=None,
                 predict_x_start=False, noise=None, learned_variance=False, clip_denoised=False):
        noise = default(noise, lambda: torch.randn_like(x_start))
    ...
    def sample(
        ...
        for unet_number, unet, vae, channel, image_size, predict_x_start, learned_variance in tqdm(
                zip(range(1, len(self.unets) + 1), self.unets, self.vaes, self.sample_channels, self.image_sizes,
                    self.predict_x_start, self.learned_variance)):
    ...
    def forward(
        ...
        return self.p_losses(unet, image, times, image_embed=image_embed, text_encodings=text_encodings,
